...
export function validateEmail(email: string): ValidationResult {
  ...
  if (trimmed.length > VALIDATION_LIMITS.MAX_EMAIL_LENGTH) {
    return { valid: false, error: `Email cannot exceed ${VALIDATION_LIMITS.MAX_EMAIL_LENGTH} characters` };
  }
 ...
 */
export function validateRedirectUrl(url: string, allowedOrigin: string): ValidationResult {
  ...
  if (trimmed.length > VALIDATION_LIMITS.MAX_URL_LENGTH) {
    return { valid: false, error: `URL cannot exceed ${VALIDATION_LIMITS.MAX_URL_LENGTH} characters` };
  }
  ...
  try {
    ...
    if (parsedUrl.protocol !== parsedOrigin.protocol || urlHost !== originHost) {
...
export function validateCheckoutItem(item: unknown, index: number): ValidationResult {
  ...
  // Validate productId
  if (typeof itemObj.productId !== 'number' || !Number.isInteger(itemObj.productId) || itemObj.productId <= 0) {
  ...
  if (itemObj.variationId !== undefined && itemObj.variationId !== null) {
    if (typeof itemObj.variationId !== 'number' || !Number.isInteger(itemObj.variationId) || itemObj.variationId <= 0) {
  ...
  if (typeof itemObj.name !== 'string' || itemObj.name.trim().length === 0) {
    return { valid: false, error: `Item ${index + 1}: Product name is required` };
  }
  ...
  if (itemObj.variation !== undefined && itemObj.variation !== null && itemObj.variation !== '') {
    if (typeof itemObj.variation !== 'string') {
      return { valid: false, error: `Item ${index + 1}: Invalid variation format` };
    }
    if (itemObj.variation.length > VALIDATION_LIMITS.MAX_VARIATION_LENGTH) {
      return { valid: false, error: `Item ${index + 1}: Variation text too long` };
    }
  }
  ...
  if (itemObj.price < VALIDATION_LIMITS.MIN_PRICE || itemObj.price > VALIDATION_LIMITS.MAX_PRICE) {
    return { valid: false, error: `Item ${index + 1}: Price out of valid range` };
  }

  // Validate quantity
  if (typeof itemObj.quantity !== 'number' || !Number.isInteger(itemObj.quantity)) {
    return { valid: false, error: `Item ${index + 1}: Invalid quantity format` };
  }
  if (itemObj.quantity < VALIDATION_LIMITS.MIN_QUANTITY || itemObj.quantity > VALIDATION_LIMITS.MAX_QUANTITY) {
    return { valid: false, error: `Item ${index + 1}: Quantity must be between ${VALIDATION_LIMITS.MIN_QUANTITY} and ${VALIDATION_LIMITS.MAX_QUANTITY}` };
  }
...
export function validateCheckoutItems(items: unknown): ValidationResult {
  ...
  if (items.length > VALIDATION_LIMITS.MAX_CART_ITEMS) {
    return { valid: false, error: `Cart cannot contain more than ${VALIDATION_LIMITS.MAX_CART_ITEMS} items` };
  }
