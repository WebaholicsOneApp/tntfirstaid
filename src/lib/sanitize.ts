...
const browserDOMPurify = typeof window !== 'undefined' ? createDOMPurify(window) : null;
...
function sanitizeHtml(
  html: string,
  config: typeof PRODUCT_DESCRIPTION_CONFIG | typeof STRICT_CONFIG | typeof TEXT_ONLY_CONFIG
): string {
  if (browserDOMPurify) {
  ...
  return clean.replace(
    ...
    (match, attrs) => {
      ...
      if (!attrs.includes('rel=')) {
        ...
        newAttrs = newAttrs.replace(/rel="([^"]*)"/, 'rel="$1 noopener noreferrer"');
      }
