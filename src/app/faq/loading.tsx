export default function FaqLoading() {
  return (
    <div className="min-h-screen bg-white">
      {/* Breadcrumb skeleton */}
      <nav className="container mx-auto px-4 py-4">
        <div className="flex items-center gap-2">
          <div className="skeleton h-3 w-10" />
          <div className="bg-secondary-200 h-3 w-px" />
          <div
            className="skeleton h-3 w-20"
            style={{ animationDelay: "60ms" }}
          />
        </div>
      </nav>

      {/* Light header skeleton (FAQ uses bg-white, not dark banner) */}
      <header className="border-secondary-100 border-b bg-white">
        <div className="container mx-auto px-4 py-12 md:py-16">
          <div className="mb-4 flex items-center gap-3">
            <div className="bg-secondary-200 h-px w-6" />
            <div className="skeleton h-2.5 w-20" />
          </div>
          <div
            className="skeleton mb-4 h-10 w-96"
            style={{ animationDelay: "100ms" }}
          />
          <div
            className="skeleton h-4 w-80 opacity-60"
            style={{ animationDelay: "200ms" }}
          />
        </div>
      </header>

      {/* Accordion sections skeleton */}
      <main className="container mx-auto px-4 py-16">
        <div className="mx-auto max-w-4xl">
          <div className="space-y-16">
            {Array.from({ length: 4 }).map((_, s) => (
              <section key={s}>
                <div className="mb-3 flex items-center gap-3">
                  <div className="bg-secondary-200 h-px w-6" />
                  <div
                    className="skeleton h-2.5 w-16"
                    style={{ animationDelay: `${s * 150}ms` }}
                  />
                </div>
                <div
                  className="skeleton mb-6 h-5 w-48"
                  style={{ animationDelay: `${s * 150 + 60}ms` }}
                />
                <div className="space-y-3">
                  {Array.from({ length: s === 0 ? 3 : 2 }).map((_, i) => (
                    <div
                      key={i}
                      className="skeleton border-secondary-50 h-12 rounded border"
                      style={{ animationDelay: `${s * 150 + 120 + i * 80}ms` }}
                    />
                  ))}
                </div>
              </section>
            ))}
          </div>

          {/* Quick links CTA skeleton */}
          <div className="mt-20 grid gap-6 md:grid-cols-3">
            {Array.from({ length: 3 }).map((_, i) => (
              <div
                key={i}
                className="border-secondary-100 border bg-white p-8 text-center"
              >
                <div
                  className="skeleton mx-auto mb-4 h-12 w-12 rounded"
                  style={{ animationDelay: `${800 + i * 120}ms` }}
                />
                <div
                  className="skeleton mx-auto mb-2 h-3 w-20"
                  style={{ animationDelay: `${860 + i * 120}ms` }}
                />
                <div
                  className="skeleton mx-auto h-3 w-36"
                  style={{ animationDelay: `${920 + i * 120}ms` }}
                />
              </div>
            ))}
          </div>
        </div>
      </main>
    </div>
  );
}
