    ...
    if (event.type === "checkout.session.completed") {
      ...
      const orderItems = await Promise.all(
        lineItems.map(async (item, index) => {
          ...
          console.log(
            `   Item ${index + 1}: ${orderItem.name} (x${orderItem.quantity}) - Price: $${(orderItem.price / 100).toFixed(2)}, Tax: $${(orderItem.tax / 100).toFixed(2)}`,
          );
          console.log(
            `      ProductId: ${productId}, VariationId: ${variationId}, Image: ${orderItem.imageUrl ? orderItem.imageUrl.substring(0, 50) + "..." : "None found"}`,
          );
      ...
      if (result) {
        ...
        if (emailResult.success) {
          console.log(
            `[WEBHOOK] Order confirmation email sent! ID: ${emailResult.messageId}`,
          );
        } else {
          console.error(
