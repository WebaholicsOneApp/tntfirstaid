...
export async function GET(request: Request) {
  ...
  try {
    ...
    const sort = (searchParams.get('sort') || 'best_sellers') as ProductSortOption;
    const pageSize = Math.min(parseInt(searchParams.get('pageSize') || '24', 10), 48);

    const parseCsvInts = (param: string | null) =>
      param ? param.split(',').map(id => parseInt(id, 10)).filter(id => !isNaN(id)) : undefined;
    ...
    const filters = {
      ...
      packAvailable: searchParams.get('packAvailable') === 'true' ? true : undefined,
